...
def get_command_output(
    protocol, shell_id, command_id, out_stream=None, err_stream=None
):
    """
    ...
    while not command_done:
        try:
            stdout, stderr, return_code, command_done = _raw_get_command_output(
                protocol, shell_id, command_id, out_stream, err_stream
            )
...
def _raw_get_command_output(
    ...
    req = {
        "env:Envelope": protocol._get_soap_header(
    ...
    stream_nodes = [
        node for node in root.findall(".//*") if node.tag.endswith("Stream")
    ]
    stdout = stderr = b""
    return_code = -1
    ...
    if command_done:
        return_code = int(
            next(
                node for node in root.findall(".//*") if node.tag.endswith("ExitCode")
            ).text
        )
