...
def run_cmd(
    ...
    rs = Response(
        self.protocol.get_command_output(
            self.protocol, shell_id, command_id, out_stream, err_stream
        )
    )
...
class RunCommand:
    def __init__(
        self, session, shell, command, retry, retry_delay, output_charset="utf-8"
    ):
